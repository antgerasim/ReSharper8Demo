...
namespace ERPStore
{
	...
	public static class IEnumerableExtension
	{
		...
		/// <summary>
		/// Permet d'ajouter une violation de regle d'integrité dans la liste
		/// </summary>
		/// <param name="brokenRules">The broken rules.</param>
		/// <param name="propertyName">Name of the property.</param>
		/// <param name="error">The error.</param>
		public static void AddBrokenRule(this IList<Models.BrokenRule> brokenRules, string propertyName, string error)
		{
			var brokenRule = brokenRules.SingleOrDefault(i => i.PropertyName.Equals(propertyName, StringComparison.CurrentCultureIgnoreCase));
			if (brokenRule == null)
			{
				brokenRule = new ERPStore.Models.BrokenRule();
				brokenRule.PropertyName = propertyName;
				brokenRules.Add(brokenRule);
			}
			brokenRule.ErrorList.Add(error);
		}
