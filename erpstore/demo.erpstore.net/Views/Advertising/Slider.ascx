<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<IList<AdvertisingView>>" %>
<div class="bloc" id="content_slide">
	<span class="numSlide"> 
	<% foreach(var item in ViewData.Model) {  %> 
        <a class="slideOff" href="javascript:gotoSlide(<%=Model.IndexOf(item)%>)"><%=Model.IndexOf(item) + 1%></a>
   <% } %>
   </span>
    ...
    <!-- 1 -->
     <div class="<%=visible%>" id="Slide<%=Model.IndexOf(item)%>">
		<% if (!item.DestinationUrl.IsNullOrTrimmedEmpty()) { %>
        <a class="lien_une" href="<%=Url.Href(item) %>">
